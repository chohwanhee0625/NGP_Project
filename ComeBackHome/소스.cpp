...
void SetgVec() // 육면체, 사면체 처음 위치 - 면 총 10개
{
	...
	Set_Ground(); // 도로 + 잔디 만들기
	makecars(); // 차 만들기
	maketrees(); // 나무 만들기
	makelines(); // 도로 흰색 라인 만들기
	makeMom(); // 도착지점 엄마 닭 만들기
...
void makeMom()
{
	...
	for (int i{}; i < size; ++i)
	{
		if (dynamic_cast<tagGrass*>(gVec[i]) != nullptr && !gVec.at(i)->Is_final()) 
		{ 
			max_z = gVec[i]->Get_zidx(); // 맨 마지막 잔디의 인덱스를 얻기 -> 엄마닭의 위치 초기화를 위함
		}
	}

	tagBodyMom* body = new tagBodyMom{ cube_vertex_array_normal, cube_color ,max_z + 5 }; // 0
	gVec.push_back(body);
	tagHeadMom* head = new tagHeadMom{ cube_vertex_array_normal, cube_color,max_z + 5 }; // 1
	gVec.push_back(head);
	tagMouseMom* mouse = new tagMouseMom{ cube_vertex_array_normal, cube_color,max_z + 5 }; // 2
	gVec.push_back(mouse);
	tagEyesMom* eyes = new tagEyesMom{ cube_vertex_array_normal, cube_color ,max_z + 5 }; // 3
	gVec.push_back(eyes);
	tagLeftArmMom* Larm = new tagLeftArmMom{ cube_vertex_array_normal, cube_color,max_z + 5 }; // 4
	gVec.push_back(Larm);
	tagRightArmMom* Rarm = new tagRightArmMom{ cube_vertex_array_normal, cube_color ,max_z +5 }; // 5
	gVec.push_back(Rarm);
	tagLeftLegMom* Lleg = new tagLeftLegMom{ cube_vertex_array_normal, cube_color ,max_z + 5 }; // 6
	gVec.push_back(Lleg);
	tagRightLegMom* Rleg = new tagRightLegMom{ cube_vertex_array_normal, cube_color,max_z +5 }; // 7
	gVec.push_back(Rleg);
